...
- **🌐 ile sıfır-kurulum dil seçimi:** Sunucuda dili olmayan bir kullanıcı 🌐'ye tıkladığında mesajın altında **satır içi (inline) dil seçme menüsü (dropdown)** açılır — en yaygın 25 dil, bayrak etiketli. Kullanıcı bir dil seçince (1) seçim `store.users[user_id].lang` olarak **kalıcı kaydedilir**, (2) mesaj anında seçilen dile çevrilip orijinalin altına **public reply** olarak yazılır; yeni üye hiç `/mylang` yazmadan dili seçip çeviriyi almış olur. Aynı kullanıcı aynı mesajda tekrar 🌐 basarsa menü yeniden açılmaz — onboarding menüsü **1 saat dedupe** edilir (spam koruması). `/mylang` geçerliliğini korur: dil **değiştirmek** ve 25 yaygın dilin dışındaki TÜM dilleri aranabilir autocomplete ile seçmek için ana yol. `/mylang` menüsünde **"Reset my language" (🚫)** düğmesi dili siler (test ve yeniden seçim için; onboarding picker'ında buton yok).
