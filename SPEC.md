...
**Kullanıcı tarafı:** Botu sunucuya ekleyen kişide **Manage Server** izni olmalı. Admin komutları `default_permissions(manage_guild=True)` ile korunur. Sunucu yöneticisi istersen bu varsayılanı değiştirebilir: **Sunucu Ayarları → Entegrasyonlar → AoEM Translator** üzerinden `/autoflag`, `/settings`, `/stats` komutlarını belirli rol veya kullanıcılara, kendilerine **Manage Server** vermeden açabilir (`default_permissions` yalnızca varsayılanı belirler, entegrasyon bazında geçersiz kılınabilir); botun kendisine ise **asla Administrator** verilmez — çalışması için altı izni yeterlidir. Kilitli kanallarda botun mesaj gönderebilmesinin öngörülen yolu kanal bazlı izin geçersiz kılmalarıdır (Kanal Ayarları → İzinler üzerinden **AoEM Translator** rolüne gerekli izinlerin açıkça verilmesi); **Administrator** izni hiçbir koşulda gerekmez ve verilmez.
...
- `/mylang [language]` — herkes kullanabilir, cevap **ephemeral**. Parametre autocomplete'li (en fazla 25 seçenek; kod, ana dil adı ve İngilizce ad ile arama: "tr", "Türkçe", "Turkish" hepsi bulur). Parametresiz çağrılırsa mevcut ayarı gösterir. Parametresiz menüde kullanıcıya bir dil ayarlıysa kırmızı **"Reset my language" (🚫)** düğmesi gösterilir; basınca kayıtlı dil (`store.users[user_id].lang`) silinir ve bir sonraki 🌐 tıklamasında onboarding dil seçme menüsü yeniden çıkar (düğme yalnızca `/mylang` menüsünde vardır, 🌐 onboarding picker'ında yoktur). Ayar kullanıcıya özeldir (sunucudan bağımsız), `store.users[user_id].lang`.
- 🌐 tepkisi: Kullanıcının dili **varsa** çeviri orijinal mesajın altına **herkese açık reply** olarak gönderilir (DM gönderilmez); dedupe anahtarı `(message_id, target, user_id)` (kişisel olduğu için kullanıcı bazlı). Dili **yoksa** bot, mesajın altına tıklanabilir bir **satır içi (inline) dil seçme menüsü (dropdown)** ekler — en yaygın 25 dil, her seçenek bayrak emoji + dil adı etiketli (eski "Set your language first with `/mylang`" uyarısının yerini bu akış aldı). Kullanıcı menüden bir dil seçtiğinde: (1) seçim `store.users[user_id].lang` olarak **kalıcı kaydedilir** (bir daha sorulmaz; sonraki 🌐 tıklamalarında doğrudan çeviri gelir), (2) mesaj **anında** seçilen dile çevrilip orijinalin altına **public reply** olarak yazılır. Böylece yeni kullanıcı sıfır kurulumla, hiç `/mylang` yazmadan 🌐'ye tıklayıp menüden seçerek dilini ayarlamış olur. Aynı kullanıcı aynı mesajda tekrar 🌐 basarsa yeni menü açılmaz: onboarding menüsü **1 saat** dedupe edilir (spam koruması). `/mylang` değişmeden kalır: dil **değiştirmek** ve bu 25 yaygın dilin dışındaki tüm dilleri (aramalı autocomplete) seçmek için ana yol.
